...
if __name__ == "__main__":
    ...
    with open("ThoiTiet_daxuly.csv",mode="w") as f:
    ...
    var_tempMax = input('Nhap nhiet do cao nhat: ')  #đoạn nhập values mà muốn dự đoán
    ...
    df.to_csv("ThoiTiet_input.csv", index=False) #tao file csv chua dl vua nhap
    
    f = open("ThoiTiet_testInput.csv", "w")
    writer = csv.writer(f) #way to write to csv file
    item = loadDataInput("ThoiTiet_input.csv") #đọc file input đấy
    for i in item:
